...
genai.configure(api_key="AIzaSyALTh88c6a6lujiG-4GATI0H_4TCmUvac8")  # Replace with your Gemini API key
...
def load_data():
    try:
        df = pd.read_excel('my_data (1).xlsx')  # Replace with your dataset file name
...
# App Header
st.markdown('<h1 class="chat-font">🤖 Rukmitha Clone Chatbot</h1>', unsafe_allow_html=True)
st.markdown('<h3 class="chat-font">Ask me anything, and I\'ll respond as Rukmitha!</h3>', unsafe_allow_html=True)
st.markdown("---")
...
def generate_refined_answer(query, retrieved_answer):
    prompt = f"""You are Rukmitha, an AI, ML, and DL instructor. Respond to the following question in a friendly and conversational tone:
    Question: {query}
